...
```
  ┌──────────────────────────┐                      ┌──────────────────────────┐
  │   AS 1  ·  10.1.0.0/16  │                      │   AS 3  ·  10.3.0.0/16  │
  │  BR_a(0)      BR_b(1)   │                      │  BR_a(0)      BR_b(1)   │
  │  └── N_1 internal ──┘   │◄────[4] 1G/5ms ─────►│  └── N_3 internal ──┘   │
  └──────┬──────────────┬───┘   metric=30 PRIMARY   └───┬──────────────┬──────┘
         │              │       FAILS at t = 20 s        │              │
[0] 1G/5ms(prim)  [1] 100M/15ms(bkp)           [2] 1G/5ms(prim) [3] 100M/15ms(bkp)
  AS1–AS2 primary   AS1–AS2 backup             AS2–AS3 primary   AS2–AS3 backup
         │              │                               │              │
         └──────┬────────┘                              └──────┬───────┘
                │                                             │
         ┌──────▼──────────────────────────────────────────────▼──────┐
         │                  AS 2  ·  10.2.0.0/16                      │
         │          BR_a(0)  ────  internal routers  ────  BR_b(1)     │
         └────────────────────────────────────────────────────────────┘
```

**Figure 1.** Inter-AS topology. The dashed link [4] (AS1.BR\_a ↔ AS3.BR\_a, metric = 30) is the direct path that fails at t = 20 s; after the simulated BGP hold-timer (2–5 s) traffic reroutes via AS 2 using the pre-installed metric = 80 backup routes.
...
| Scenario | N | Distribution | N_1 (AS1) | N_2 (AS2) | N_3 (AS3) | Flows |
|---------:|--:|:-------------|----------:|----------:|----------:|------:|
...
**Table 5.** Scenario matrix. N_i computed by: balanced = ceil(N/3) / ceil(N/3) / remainder; unbalanced = round(N×0.2) / round(N×0.3) / remainder. Flows = 4 + floor(N/10).
...
| Extra (cycling) | AS2→AS1, AS3→AS2, then repeating base dirs | floor(N/10) |
...
\toprule
\textbf{Nodes} & \textbf{Distribution} & \textbf{Delay (ms)} & \textbf{Throughput (Mbps)} & \textbf{Loss (\%)} & \textbf{Conv.\ (s)} \\
\midrule
 20 & balanced   & $16.98 \pm 2.95$ & $5.09 \pm 0.00$ & $0.002 \pm 0.001$ & $2.78 \pm 0.85$ \\
 20 & unbalanced & $16.93 \pm 1.65$ & $4.98 \pm 0.19$ & $0.003 \pm 0.000$ & $2.78 \pm 0.85$ \\
 50 & balanced   & $23.63 \pm 0.74$ & $5.09 \pm 0.00$ & $0.000 \pm 0.001$ & $3.43 \pm 1.38$ \\
 50 & unbalanced & $17.39 \pm 1.80$ & $5.09 \pm 0.00$ & $0.002 \pm 0.001$ & $3.43 \pm 1.38$ \\
100 & balanced   & $20.47 \pm 3.14$ & $5.04 \pm 0.09$ & $0.002 \pm 0.000$ & $3.13 \pm 0.85$ \\
...
A larger AS contains more internal nodes, so the expected path length from a non-BR source to the nearest BR grows approximately as floor(log2(N_i/2)) in the balanced two-group tree.
For the 20-node balanced scenario (N_1 = N_2 = 7), the typical intra-AS path length is 2–3 hops (4–6 ms).
For the 100-node balanced case (N_1 = 34), it extends to 4–5 hops (8–10 ms), yielding an increase of roughly **4–6 ms** in expected one-way intra-AS delay.
...
From Table 8 (Figure 2), the measured mean delay increases from approximately **16.98 ms** (N=20, balanced) to **20.47 ms** (N=100, balanced), a relative increase of roughly **20.5%**. This is consistent with the hop-count analysis above: the intra-AS path grows with network size while the fixed 5 ms inter-AS hop cost dominates at small N.
...
Measured scenario-average loss is **0.002%** across all scenarios. This is lower than a naive black-hole estimate would predict because ns-3's `Ipv4::SetDown()` drains the queue cleanly at the interface level, and the FlowMonitor counts only packets that are transmitted but never received — not packets withheld before transmission. Flows not crossing link[4] (AS1↔AS2, AS2↔AS3 directions) show zero loss throughout.
...
In the unbalanced case, AS3 receives 50% of all nodes (e.g., N_3 = 50 for N = 100), making it the dominant source of intra-AS hop accumulation.
...
From Table 8, the mean delay for unbalanced scenarios is **higher** than balanced at N=100 (23.65 ms vs. 20.47 ms, a 15.5% increase), and comparable at N=20 (16.93 vs. 16.98 ms). This is consistent with the prediction: at large N, AS3 hosts 50 nodes vs. 32 in the balanced case, extending intra-AS paths for flows terminating in AS3.
